    ...
    B: tower_h2::Body + 'static,
{
    ...
    fn bind(&self, ep: &Endpoint) -> Result<Self::Service, Self::BindError> {
        Ok(self.bind.new_bound_service(ep, &self.protocol))
    }
}
    ...
    fn poll_ready(&mut self) -> Poll<(), Self::Error> {
        ...
        match ready {
            Err(ReconnectError::Connect(err)) => {
                ...
                // So, this service isn't "ready" now, but to only make the
                // service try another connect if there are still requests
                // waiting, this doesn't poll the newly bound service.
                //
